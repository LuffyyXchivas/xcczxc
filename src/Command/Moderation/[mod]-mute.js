...
const emoji = require('../../Settings/Emojis.json');
    ...
    .setDescription(`
   ${emoji.kevzyymute2} ${uye} (\`${uye.id}\`) Adlı Kullanıcı Metin Kanallarında Susturuldu!

    ${emoji.kevzyyok} **Yetkili:** ${message.author} (\`${message.author.id}\`)
    ${emoji.kevzyyok} **Cezalı:** ${uye} (\`${uye.id}\`)
    ${emoji.kevzyyzaman} **Süre:** \`${zaman}\`
    ${emoji.kevzyyicon} **Mute Atılma:** \`${muteatılma}\`
    ${emoji.kevzyyicon} **Mute Bitiş:** \`${mutebitiş}\`

    ${emoji.Book} **Sebep:** \`${sebep}\`
    `)
    client.channels.cache.get(config.Log.MuteLog).send(anen)
    await uye.roles.add(config.Roller.Muted)
    message.channel.send(new MessageEmbed().setAuthor(message.member.displayName, message.author.avatarURL({dynamic: true})).setDescription(`${uye} (\`${uye.id}\`) adlı kullanıcı sunucumuzda mutelendi!`).setColor("BLACK").setFooter(Main.Footer))
    ...
    setTimeout(async() => {
        ...
        if(!muteli) return;
        if(muteli == 'muteli') {
            client.channels.cache.get(config.Log.MuteLog).send(new MessageEmbed().setFooter(Main.Footer).setAuthor(message.member.displayName, message.author.avatarURL({dynamic: true})).setFooter(Main.Footer).setColor("GREEN").setDescription(`
    ${emoji.kevzyyunmute} ${uye} (\`${uye.id}\`) Adlı Kullanıcı Metin Kanallarında Susturması Bitti!

...
const emoji = require('../../Settings/Emojis.json');
    ...
    .setColor("ffffff")
    .setDescription(`
    ${emoji.kevzyykevzyymute} ${uye} (\`${uye.id}\`) Adlı Kullanıcı Ses Kanallarında Susturuldu!

    ${emoji.kevzyyok} **Yetkili:** ${message.author} (\`${message.author.id}\`)
    ${emoji.kevzyyok} **Cezalı:** ${uye} (\`${uye.id}\`)
    ${emoji.kevzyyzaman} **Süre:** \`${zaman}\`
    ${emoji.kevzyyicon} **Mute Atılma:** \`${voiceatılma}\`
    ${emoji.kevzyyicon} **Mute Bitiş:** \`${voicebitiş}\`
    ...
    await client.channels.cache.get(config.Log.CezaPuanLog).send(`${emoji.kevzyy} ${uye}: aldığınız **#${cezaID}**  ID'li ceza ile **${cpuan}** ceza puanına ulaştınız.`)

    setTimeout(async() => {
        let vmuteli = db.fetch(`vmuteli.${uye.id}.${message.guild.id}`)
        if(!vmuteli) return;
        if(vmuteli == 'vmuteli') {
            client.channels.cache.get(config.Log.VMuteLog).send(new MessageEmbed().setFooter(Main.Footer).setColor("GREEN").setAuthor(message.member.displayName, message.author.avatarURL({dynamic: true})).setDescription(`
    ${emoji.voice} ${uye} (\`${uye.id}\`) Adlı Kullanıcı Ses Kanallarında Susturması Bitti!

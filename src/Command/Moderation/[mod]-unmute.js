...
module.exports.beta = async(client, message, args) => {
  ...
  let atılmaay = moment(Date.now()).format("MM")
    ...
    client.channels.cache.get(config.Log.MuteLog).send(new MessageEmbed().setFooter(Main.Footer).setAuthor(message.member.displayName, message.author.avatarURL({dynamic: true})).setColor("GREEN").setDescription(`
    ${emoji.kevzyyunmute} ${uye} (\`${uye.id}\`) Adlı Kullanıcının Chat Susturulması Kaldırıldı!

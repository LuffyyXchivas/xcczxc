...
module.exports.beta = async(client, message, args) => {
    let embed = new MessageEmbed().setColor("GREEN").setFooter(Main.Foooter)
...
const emoji = require('../../Settings/Emojis.json');
    const kaldırma = embed.setAuthor(message.member.displayName, message.author.avatarURL({dynamic: true})).setDescription(`

   ${emoji.kevzyyban} <@!${uye}> Adlı Kullanıcı Sunucumuzdan Yasağı Kaldırıldı!

   ${emoji.kevzyyok} **Yetkili:** ${message.author} (\`${message.author.id}\`)
   ${emoji.kevzyyok} ** Yasaklı:** ${uye} (\`${uye.id}\`)
   ${emoji.kevzyyzaman} **Tarih:** \`${banatılma}\`
    
    ${emoji.Book} **Sebep:** \`${sebep}\`
    `)
message.channel.send(new MessageEmbed().setAuthor(message.member.displayName, message.author.avatarURL({dynamic: true})).setDescription(`<@!${uye}> (\`${uye}\`) adlı kullanıcı sunucumuzda yasağı kaldırıldı!`).setColor("BLACK").setFooter(Main.Footer))
